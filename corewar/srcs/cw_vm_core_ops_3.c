...
/*   Updated: 2019/12/20 00:23:14 by gbrandon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		sti(t_vm *vm, t_prcs *prcs)
{
	...
	//ft_printf("VAL2: %d\n", val1);
	vm_writebreg(vm->field, vm_add_address(prcs->pc, val1), prcs->regs[reg - 1], REG_SIZE);
}
...
void		lld(t_vm *vm, t_prcs *prcs)
{
	...
	if (argtype == T_DIR)
	{
		val = ops_read_tdir(vm->field, vm_add_address(prcs->pc, 1 + 1), DIR_SIZE);
		//ft_printf("VAL1: %d\n", val);
		prcs->carry = ((val) ? 0 : 1);
		reg = ops_read_treg(vm->field, vm_add_address(prcs->pc, 1 + 1 + DIR_SIZE));
		//ft_printf("reg_num: %d\n", reg);
	...
	{
		val = ops_read_tdir(vm->field, vm_add_address(prcs->pc, 1 + 1), IND_SIZE);
		//ft_printf("IND_VAL: %d\n", val);
		reg = ops_read_treg(vm->field, vm_add_address(prcs->pc, 1 + 1 + IND_SIZE));
		//ft_printf("reg_num: %d\n", reg);
		val = ops_read_tdir(vm->field, vm_add_address(prcs->pc, val), DIR_SIZE);
		//ft_printf("VAL1: %x\n", val);
		prcs->carry = ((val) ? 0 : 1);
		(prcs->regs)[reg - 1] = r_mov((prcs->regs)[reg - 1], &val, DIR_SIZE);
		//ft_printf("reg_value: %x\n", r_geti(prcs->regs[reg - 1]));
